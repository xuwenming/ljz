...
<script type="text/javascript">
	$(function() {
	 parent.$.messager.progress('close');
		$('#form').form({
			url : '${pageContext.request.contextPath}/ljzPrizeLogController/add',
			onSubmit : function() {
		...
		<form id="form" method="post">		
			...
			<table class="table table-hover table-condensed">
				<tr>	
					...
					<td>
					<input class="span2" name="addtime" type="text" onclick="WdatePicker({dateFmt:'<%=TljzPrizeLog.FORMAT_ADDTIME%>'})"  maxlength="0" class="required " />
					</td>							
					<th><%=TljzPrizeLog.ALIAS_UPDATETIME%></th>	
					<td>
					<input class="span2" name="updatetime" type="text" onclick="WdatePicker({dateFmt:'<%=TljzPrizeLog.FORMAT_UPDATETIME%>'})"  maxlength="0" class="required " />
					</td>							
				</tr>	
				<tr>	
