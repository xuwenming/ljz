...
<c:if test="${fn:contains(sessionInfo.resourceList, '/ljzWithdrawLogController/editAudit')}">
	<script type="text/javascript">
		$.canEditAudit = true;
	</script>
</c:if>
	<c:if test="${fn:contains(sessionInfo.resourceList, '/ljzWithdrawLogController/viewStatus')}">
		<script type="text/javascript">
			$.canViewStatus = true;
		</script>
	</c:if>
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			...
			columns : [ [ {
				...
				width : 50,
				formatter: function (value, row, index) {
					if(row.refType == 'BBT006') return '提现-用户端';
					else return '提现-店铺端';
				}
			}, {
				field : 'action',
	...
	function editAuditFun(id) {
		...
		parent.$.modalDialog({
			...
			width: 780,
			height: 200,
			href: '${pageContext.request.contextPath}/ljzWithdrawLogController/editAuditPage?id=' + id ,
			buttons: [{
				text: '同意',
				handler: function () {
	...
	function viewStatus(withdrawNo) {
		$.post('${pageContext.request.contextPath}/ljzWithdrawLogController/viewStatus', {
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="display: none;">
					<tr>
						...
						<th>申请时间</th>
						<td colspan="3"><input class="span2" name="addtimeStart"
											   placeholder="点击选择时间"
											   onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})"
											   readonly="readonly" />至<input class="span2"
																			 name="addtimeEnd" placeholder="点击选择时间"
																			 onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})"
