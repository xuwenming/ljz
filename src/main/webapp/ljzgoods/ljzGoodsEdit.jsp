		...
		<form id="form" method="post">
			...
			<table class="table table-hover table-condensed">
				<tr>
					...
					<td colspan="3">
						<input value="${ljzGoods.title}" class="easyui-validatebox span2" data-options="required:true" maxlength="100" name="title" type="text" style="width: 510px;"/>
					</td>
				</tr>
				<tr>
					<th width="10%">所属店铺</th>
					<td width="40%">
						<jb:selectSql dataType="SQL001" name="shopId" value="${ljzGoods.shopId}"></jb:selectSql>
					</td>
					<th width="10%">状态</th>
					<td width="40%">
						<select name="isPutAway" class="easyui-combobox"
								data-options="width:140,height:29,editable:false,panelHeight:'auto'">
							<option value="0" <c:if test="${!ljzGoods.isPutAway}">selected</c:if>>上架</option>
							<option value="1" <c:if test="${ljzGoods.isPutAway}">selected</c:if>>下架</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input value="${ljzGoods.price}" class="easyui-validatebox span2" data-options="required:true" maxlength="10" name="price" type="text"/>
					</td>
					<th><%=TljzGoods.ALIAS_FREIGHT%></th>
					<td>
						<input class="span2" name="freight" maxlength="10" value="${ljzGoods.freight}" type="text"/>
					</td>
				...
				<tr>
					<th>库存数量</th>
					<td>
						<input name="quantity" value="${ljzGoods.quantity}"
					...
					<td>
						<input name="limitNumber" value="${ljzGoods.limitNumber}"
							   class="easyui-numberspinner" style="width: 140px; height: 29px;"
							   required="required" data-options="editable:true" maxlength="10"/>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<input class="easyui-validatebox span2" maxlength="10" value="${ljzGoods.shareAmount}" data-options="required:true" name="shareAmount" type="text"/>
					</td>
				</tr>
				<tr>
					<th><%=TljzGoods.ALIAS_PRIZE_PRE%></th>
					<td>
						<input name="prizePre" value="${ljzGoods.prizePre}"
							   class="easyui-numberspinner" style="width: 140px; height: 29px;"
							   required="required" data-options="editable:true" maxlength="10" />
					</td>
					<th><%=TljzGoods.ALIAS_PRIZE_NUMBER%></th>
					<td>
						<input name="prizeNumber" value="${ljzGoods.prizeNumber}"
							   class="easyui-numberspinner" style="width: 140px; height: 29px;"
							   required="required" data-options="editable:true" maxlength="10" />
					</td>
				</tr>
				<tr>
					<th width="10%">商品说明</th>
					<td colspan="3">
						<textarea name="describtion" class="easyui-validatebox" data-options="required:true"  style="width: 510px">${ljzGoods.describtion}</textarea>
					</td>
				</tr>
				<tr>
					<td colspan="4">
						...
						<div id="imageQueue" style="margin-top:5px;">
							<c:forEach items="${ljzGoods.imageUrls}" var="image" varStatus="status">
								<div id="SWFUpload_0_${status.index+99}" class="uploadify-queue-item">
									<div class="cancel">
										<a href="javascript:cancel('SWFUpload_0_${status.index+99}')">X</a>
									</div>
									<span class="fileName" filename="${image}">
										图片${status.index + 1}
									</span>
									<span class="data complete"> - 上传成功</span>
								</div>
							</c:forEach>
						</div>
					</td>
				...
				<tr>
					<td colspan="4" height="155" class="img-preview">
						<c:forEach items="${ljzGoods.imageUrls}" var="image" varStatus="status">
							<img src="${image}" fileid="SWFUpload_0_${status.index+99}" width="150"/>
						</c:forEach>
					</td>
				</tr>
			</table>
